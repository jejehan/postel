...
Partial Class UserControls_ucForm
    ...
    Sub GetMemberList()
        ...
        Dim Query As String = ""
        Dim dt As DataTable
        Query = "SELECT * FROM DataIKRAP WHERE CallSign=" & Trim(CallSign)
        ...
        With dt
            If .Rows.Count > 0 Then
                ...
                TbxRapiDaerah.Text = Trim(.Rows(0)("RapiDaerahName"))
                TbxNmrIKRAP.Text = Trim(.Rows(0)("NomorIKRAP"))
                TbxTglBerlakuIKRAP.Text = Trim(.Rows(0)("MasaBerlakuIKRAP"))
    ...
    Protected Sub BtnCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnCetak.Click
        ...
        'Dim TanggalBerlakuIKRAP As String = ""
        'Dim NomorIKRAP As String = ""
        'Dim FileFoto As String = ""
        'Dim dt As New DataTable
        'Dim cn As New OleDbConnection(MyDatabase)
        'Dim da As New OleDbDataAdapter("SELECT * FROM [DataIKRAP] WHERE CallSign=" & CallSign & " ", cn)
        'da.Fill(dt)
        'With dt
        '    If .Rows.Count > 0 Then
        '        GroupID = .Rows(0)("GroupID")
        '        ProsesID = "REPRINT_" & Replace(CallSign, "'", "")
        '        Nama = .Rows(0)("NamaKartu")
        '        Alamat1 = .Rows(0)("Alamat1Kartu")
        '        Alamat2 = .Rows(0)("Alamat2Kartu")
        '        Alamat3 = .Rows(0)("Alamat3Kartu")
        '        TanggalBerlakuIKRAP = .Rows(0)("MasaBerlakuIKRAP")
        '        NomorIKRAP = .Rows(0)("NomorIKRAP")
        '        FileFoto = .Rows(0)("FileFoto")
        '        DataIKRAP = "'" & GroupID & "','" & ProsesID & "','" & Nama & _
        '                    "'," & CallSign & ",'" & Alamat1 & _
        '                    "','" & Alamat2 & "','" & Alamat3 & "','" & TanggalBerlakuIKRAP & _
        '                    "','" & NomorIKRAP & "','" & FileFoto & "' "
        '        s = qInsertDataIKRAP(DataIKRAP)
        '        s = InsertDataProcess(s)

        '        Dim NamaFoto As String = MapPath("~/FileFoto/") & Trim(FileFoto)
        '        s = qSaveImageToDB(NamaFoto, ProsesID, "DataIKRAP")
        '        If s <> "True" Then
        '            Message = "Convert Image Gagal"
        '            s = qDeleteDataIKRAP(ProsesID)
        '        Else
        ...
        With dt
            If .Rows.Count > 0 Then
                ...
                DataIAR = "'" & GroupID & "','" & ProsesID & "','13','Rapi','" & CallSign & "','','" & Nama & _
